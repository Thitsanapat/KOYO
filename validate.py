...
DEFAULT_OBSERVATIONS = ("14468821", "14526577")
...
def load_frames(observation_ids):
    all_f = []
    for obs_id in observation_ids:
        path = ROOT / "data" / "koyo" / "frames_hex" / f"{obs_id}.txt"
        if not path.exists():
            raise FileNotFoundError(f"Reference frames not found: {path}")
        for index, line in enumerate(path.read_text(encoding="utf-8").splitlines(), start=1):
            frame = bytes.fromhex(line.strip())
            all_f.append((f"obs {obs_id}, frame {index}", frame))
    return all_f
...
def main():
    ...
    parser.add_argument(
        ...
        help="SatNOGS observation ID to validate; may be supplied more than once",
    )
    args = parser.parse_args()
    all_f = load_frames(args.observation_ids or DEFAULT_OBSERVATIONS)
    print(f"Validating {len(all_f)} frames from {len(args.observation_ids or DEFAULT_OBSERVATIONS)} observation(s)")

    for label, f in all_f:
        d = parse(f)
        print(f"\n{'='*58}\nFrame received {label}\n{'='*58}")
